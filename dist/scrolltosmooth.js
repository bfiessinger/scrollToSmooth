  ...
  class scrollToSmooth {
    ...
    linkCollector() {
      ...
      Array.prototype.forEach.call(this.elements, el => {
        let baseURI = getBaseURI(el);
        let targetSelector = this.getTargetElement(el); // Check if the selector is found on the page
    ...
    handleEvents(action, linksFiltered) {
      Array.prototype.forEach.call(linksFiltered, link => {
        if (action == "add") {
          link.addEventListener(
            "click",
            this.clickHandler.bind(this, link),
            false
          );
